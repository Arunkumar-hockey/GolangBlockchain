...
func NewWallet() *Wallet {
	...
	digest2 := h2.Sum(nil)
	//3. Perform RIPEMD=256 hashing on the result of SHA-256 (20 bytes)
	h3 := ripemd160.New()
	h3.Write(digest2)
	digest3 := h3.Sum(nil)
	...
	vd4[0] = 0x00
	copy(vd4[1:], digest3[:])
	//5. Perform SHA-256 hash on the extend RIPEMD-160 result
	h5 := sha256.New()
	h5.Write(vd4)
	digest5 := h5.Sum(nil)
	//6. PerformSHA-256 hash on the result of the previous SHA-256 hash
	h6 := sha256.New()
	h6.Write(digest5)
	digest6 := h6.Sum(nil)
